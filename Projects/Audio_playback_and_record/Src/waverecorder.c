...
void BSP_AUDIO_IN_TransferComplete_CallBack(void)
{
  ...
  memcpy(&WrBuffer[(ITCounter * (INTERNAL_BUFF_SIZE/8))], RecBuf, PCM_OUT_SIZE*2);
  
  BufferCtl.offset = BUFFER_OFFSET_NONE;
  
  if(ITCounter == (WR_BUFFER_SIZE/(INTERNAL_BUFF_SIZE/4))-1)
  {
    AUDIODataReady = 1;
    AUDIOBuffOffset = 0;
    ITCounter++;
  }
  else if(ITCounter == (WR_BUFFER_SIZE/(INTERNAL_BUFF_SIZE/8))-1)
  {
    AUDIODataReady = 1;
