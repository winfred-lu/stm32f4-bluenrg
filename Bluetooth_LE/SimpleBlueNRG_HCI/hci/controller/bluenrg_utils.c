...
int read_IFR(uint8_t *data)
{
  ...
  aci_updater_start(); // TODO: Use HW activation if possible 
  if(aci_get_updater_version(&version))
    return BLE_UTIL_ACI_ERROR;
...
void parse_IFR_data_config(const uint8_t data[64], IFR_config2_TypeDef *IFR_config)
{
  ...
  //IFR_config->ls_crystal_period = LE_TO_HOST_32(data+20);
  //IFR_config->ls_crystal_freq = LE_TO_HOST_32(data+24);
...
void change_IFR_data_config(IFR_config2_TypeDef *IFR_config, uint8_t data[64])
{
  ...
  //HOST_TO_LE_32(data+20, IFR_config->ls_crystal_period);
  //HOST_TO_LE_32(data+24, IFR_config->ls_crystal_freq);
